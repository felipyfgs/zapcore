...
func NewMediaHandler(mediaRepo *repository.MediaRepository, minioClient *storage.MinIOClient) *MediaHandler {
	return &MediaHandler{
		mediaRepo:     mediaRepo,
		validationSvc: service.NewMediaValidationService(),
...
func (h *MediaHandler) UploadMedia(w http.ResponseWriter, r *http.Request) {
	...
	r.Body = http.MaxBytesReader(w, r.Body, domain.MaxDocumentSize+1024*1024)

	// Parse do multipart form
	if err := r.ParseMultipartForm(domain.MaxDocumentSize); err != nil {
	...
	// Valida o arquivo
	mediaFile, err := h.validationSvc.ValidateFile(file, header)
	if err != nil {
		log.Error().Err(err).Str("filename", header.Filename).Msg("Erro na validação do arquivo")
		h.writeErrorResponse(w, http.StatusBadRequest, "INVALID_FILE", err.Error(), "file")
		return
	}

	// Gera ID único para o arquivo
	mediaFile.ID = uuid.New().String()

	// Sanitiza o nome do arquivo
	if customFilename := r.FormValue("filename"); customFilename != "" {
		mediaFile.Filename = h.validationSvc.SanitizeFilename(customFilename)
	} else {
		mediaFile.Filename = h.validationSvc.SanitizeFilename(header.Filename)
	}

	// Adiciona extensão se não tiver
	if filepath.Ext(mediaFile.Filename) == "" {
		ext := h.validationSvc.GetFileExtension(mediaFile.MimeType)
	...
	// Resposta de sucesso
	response := domain.MediaUploadResponse{
		Success: true,
